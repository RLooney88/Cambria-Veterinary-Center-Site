# Cambria Veterinary Center - Smart Site

## 1. Original Problem Statement
Rebuild example-vet-site.com as a **Smart Site** that tracks visitor intent (Signals) to dynamically swap content (Switches) in designated areas (Surfaces). Use PostgreSQL on Railway, JWT auth, Dribbble "Pet Care" template layout with Cambria Vet brand colors.

## 2. Architecture
- **Backend**: FastAPI + SQLAlchemy async + Postgres (Railway). JWT admin auth + JWT client portal auth. SendGrid email (pending key). Chatbot via LiteLLM/OpenAI-compatible provider (GPT-4o-mini). Webhook outbound API.
