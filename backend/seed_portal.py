...
async def seed_portal():
    async with AsyncSessionLocal() as db:
        ...
        # Appointments
        db.add(PetAppointment(pet_id=rosie.id, date="2025-06-10", reason="Annual Wellness Exam + Vaccines", provider="the Cambria Veterinary Center team", status="completed", notes="All vaccines updated. Weight stable. Heart and lungs clear."))
        db.add(PetAppointment(pet_id=rosie.id, date="2025-09-18", reason="Bordetella Booster", provider="the Cambria Veterinary Center team", status="completed", notes="Quick visit. No concerns."))
        db.add(PetAppointment(pet_id=rosie.id, date="2024-11-05", reason="Dental Cleaning", provider="the Cambria Veterinary Center team", status="completed", notes="Dental cleaning under anesthesia. Recovered well."))
        db.add(PetAppointment(pet_id=rosie.id, date="2026-06-10", reason="Annual Wellness Exam", provider="the Cambria Veterinary Center team", status="upcoming"))
